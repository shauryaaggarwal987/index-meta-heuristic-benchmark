...
/*
  ...
  3) HBSI (Hash-Bucketed String Index) on Reg No

  Options:
  - --data=students_100k.csv   -> load dataset from CSV instead of static header
  - --n=10000                  -> run benchmark on only N records after deterministic shuffle
  - --csv=results/x.csv        -> append 3 rows (FQA/BurstTrie/HBSI) to a CSV file
  - --opt=Baseline/ABC/PSO/GWO -> label written into CSV

  Example:
    g++ -O2 -std=gnu++17 benchmarking_code_new.cpp -o bench
    ./bench --data=students_100k.csv --n=10000 --opt=Baseline --csv=results/dataset_10k.csv
*/
...
// Optional generated static dataset header.
// Default repo mode uses CSV loading so the code compiles quickly with -O2.
// To enable the static header fallback, compile with: -DUSE_STATIC_DATA
#if defined(USE_STATIC_DATA) && __has_include("students_data.hpp")
#include "students_data.hpp" // inline vector<Student> load_students_static()
#define HAS_STATIC_STUDENTS 1
#else
#define HAS_STATIC_STUDENTS 0
static vector<Student> load_students_static() { return {}; }
#endif
...
int main(int argc, char **argv) {
    ...
    if (args.count("data")) {
        ...
        // Repo-friendly default: try students_100k.csv from the current working directory.
        students = load_students_csv("students_100k.csv");
        if (!students.empty()) {
            cout << "Loaded students from CSV: " << students.size() << "\n";
        } else {
#if HAS_STATIC_STUDENTS
            students = load_students_static();
            if (students.empty()) {
                cerr << "ERROR: Static dataset is empty. Use --data=students_100k.csv or regenerate students_data.hpp.\n";
                return 1;
            }
            cout << "Loaded students (static): " << students.size() << "\n";
#else
            cerr << "ERROR: Could not load students_100k.csv from current directory.\n";
            cerr << "Provide --data=path/to/students_100k.csv.\n";
            return 1;
#endif
        }
    }
    ...
    double hbsi_build_ms = t.toc_ms();
    size_t hbsi_mem = hbsi.approx_bytes();
    ...
    cout << "[HBSI] build: " << hbsi_build_ms << " ms | reps=" << hbsi.reps.size()
         << " | buckets=" << hbsi.buckets.size()
         << " | approx mem: " << hbsi_mem << " (" << human_bytes(hbsi_mem) << ")\n";
    ...
    t.tic();
    for (int i = 0; i < Q; i++) {
        int idx = hbsi.find_exact(reg_queries[i]);
        sink += (idx != -1);
    }
    double hbsi_query_ms = t.toc_ms();
    cout << "[HBSI] query: " << hbsi_query_ms << " ms\n";
    ...
    if (!csv.empty()) {
        ...
        append_row_csv(csv, "HBSI", opt, hbsi_build_ms, hbsi_query_ms, hbsi_mem);
